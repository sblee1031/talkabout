...
/*  $("#btnComment").show();
  ...
  $("#copytablecommentList").show();
  $("#insert_btn").show();
  $("#btnupdate").show();
  $("#imglike").show();
  $("#imgdislike").show();
  $("#trinsert").attr("readonly", true);
  var url = "localhost:9999/ta_back/login";*/
  $.ajax({
    url: url,
    method: "post",
    data: {},
  ...
  $("#debateList").on("click", "#debate_no", function () {
    ...
    $.ajax({
      ...
      method: "get",
      success: function (resposeData) {
		var debate_no = resposeData.dabateone.debate_no;
		var debate_writer = resposeData.dabateone.debate_writer;
		var debate_topic = resposeData.dabateone.debate_topic;
		var debate_date = resposeData.dabateone.debate_date;
		var debate_discuss = resposeData.dabatetwo[0].discuss;
		var debate_discuss2 = resposeData.dabatetwo[1].discuss;
		var debate_evi1_1 = resposeData.dabatetwo[0].evi_one;
		var debate_evi1_2 = resposeData.dabatetwo[0].evi_two;
		var debate_evi1_3 = resposeData.dabatetwo[0].evi_three;
		var debate_evi2_1 = resposeData.dabatetwo[1].evi_one;
		var debate_evi2_2 = resposeData.dabatetwo[1].evi_two;
		var debate_evi2_3 = resposeData.dabatetwo[1].evi_three;
		console.log(debate_discuss);
		
		$("div.debateboardlistbox").empty();
		var detailboard ='<div class="resultboard">'+
       ' <div>'+
            '<h3 class="debboard_debatettitle">'+debate_topic+'</h3>'+
        '</div>'+
       ' <div class="sublist">'+
           ' <table class="debboard_sub">'+
                '<tr>'+
                   ' <td style="font-weight: 700;">'+debate_writer+'</td>'+
                    '<td style="font-size: 5px; color: rgb(110, 110, 110);">'+debate_date+'</td>'+
          ' </table>'+
        '</div>'+
       '<div>'+
            '<table class="content" style="width: 100%;">'+
                '<tr>'+
                    '<td>'+"주장1 : "+debate_discuss+'</td>'+
                    '<td>'+"주장2 : "+debate_discuss2+'</td>'+
                '</tr>'+
                '<tr>'+
                    '<td class="reason">'+"근거1 : "+debate_evi1_1+'</td>'+
                    '<td class="reason">'+"근거1 : "+debate_evi2_1+'</td>'+
               '</tr>'+
                '<tr>'+
                    '<td class="reason">'+"근거2 : "+debate_evi1_2+'</td>'+
                    '<td class="reason">'+"근거1 : "+debate_evi2_2+'</td>'+
                '</tr>'+
                '<tr>'+
                    '<td class="reason">'+"근거3 : "+debate_evi1_3+'</td>'+
                    '<td class="reason">'+"근거1 : "+debate_evi2_3+'</td>'+
                '</tr>'+
            '</table>'+
        '</div>'+
        '<div>댓글입력용 div</div>'+
	...
	$('#section').append(detailboard);
      ...
      /*  var debate = resposeData.dabateone;
       ...
       /* var discussor1 = dismem[0].member_nickName;
        ...
        /*if (
          userdata.logined == "logined" &&
          writer == userdata.member.member_no
        ) {
      ...
      success: function (resposeData) {
        ...
        if (lists == "" || lists == null || typeof lists == "undefined") {
          ...
          $(lists).each(function (list_i, list) {
            ...
            copycomlist
              .find("#deb_writter")
              .html(mlists[list_i].member_nickName);
            ...
            if (
              userdata.logined == "logined" &&
              mlists[list_i].member_no == userdata.member.member_no
            ) {
